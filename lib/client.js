 ...
 */
function Client(baseUrl, user, pass, amqpURI, appName) {
  var self = this;
  events.EventEmitter.call(self);

  this.proxy = (amqpURI !== undefined ? true : false);
  this.url = baseUrl;
  this._swagger;
  this._proxy;
  this._ws;
  ...
  return new Promise(function(resolve, reject) {
    ...
    function swaggerLoaded () {
      if(self._swagger.ready === true) {
        ...
        if (self.proxy) {
          self._proxy = new Amqp(self._amqpURI, self._appName);
    ...
    function processMessage (msg, flags) {
      ...
      _.each(eventModel.properties, function (prop) {
        if (_.contains(_resources.knownTypes, prop.dataType) &&
          ...
          instance._dialogId = event.proxyDialogId || null;
      ...
      function SessionClient (dialogId) {
        this.dialogId = dialogId;
      }
      var sessionClient;
      if (event.type === 'StasisStart') {
        //create a new client which points back at the original client
        sessionClient = new SessionClient(event.proxyDialogId);
        Object.getOwnPropertyNames(self).forEach(function (key) {
          if (self[key] instanceof Function) {
            sessionClient[key] = function(...args) {
              return self[key].call(sessionClient, ...args, event.proxyDialogId);
            };
          } else {
            Object.defineProperty(sessionClient, key, {
              get: function() { return self[key] },
              set: function(y) { self[key] = y; }
            });
          }
        });
        self.emit(event.type, event, resources, sessionClient);     
      } else {
...
module.exports.connectProxy = function (baseUrl, user, pass, amqpURI, appName,
    /**
     *  @callback connectCallback
     *  @memberof module:ari-client
     *  @param {Error} err - error object if any, null otherwise
     ...
     */
    callback) {
  var client = new Client(baseUrl, user, pass, amqpURI, appName);
  client.proxy = true;
  client.setMaxListeners(0);
